...
import agux_non_linear_plot_regressor_v1_0_0_gui_file as GUI
from agux_non_linear_plot_regressor_v1_0_0_gui_file import *
...
class MyMPLFrame1(GUI.MyFrame1):
	...
	def wx_pop_error_est(self,event):
		if self.m_radioBox31.GetSelection()==1:
			self.m_panel6.Show()
			self.m_splitter1.SetSashPosition(316)
		else:
			self.m_panel6.Hide()
			self.m_splitter1.SetSashPosition(314)
	...
	def wxfit_custom (self,event):
		...
		if self.tipos_de_fiteo.GetSelection()==0:
			...
			self.m_splitter1.SetSashPosition(317)
...
# dialog.Show(True)
frame.cargar_mpl()
frame.m_splitter1.SetSashPosition(316) # Es un workaroud, al mover el sash, maximizar, o minimizar se corrige el bug de que el panel de MPL no aparece, originalmente está en 150 el sash, lo corro a 151 apenas empieza y listo.
